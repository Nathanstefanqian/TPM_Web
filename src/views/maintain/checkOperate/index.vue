<template>
  <div class="app-container list">
    ...
    <el-table ref="listTable" v-loading="loading.table" v-adaptive="{ bottomOffset: 55 }" height="200px" :data="datas" :default-sort="sort" border fit highlight-current-row @sort-change="handleSort">
      ...
      <el-table-column fixed="right" label="操作" align="center" width="180">
        <template slot-scope="{row}">
          ...
          <!--          </el-tooltip>-->
          <el-tooltip transition="false" :hide-after="1000" class="item" content="点检" placement="top-end">
            <el-button type="primary" plain class="button-operate button-update" size="mini" @click="handleUpdate(row)"><i class="vue-icon-update" /></el-button>
          </el-tooltip>
    ...
    <dialog-check-device ref="dialogCheckDevice" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...crud,
    handleQueryDay() {
      this.query.maintainType = '日'
      this.handleQuery()
    },
    handleQueryMonth() {
      this.query.maintainType = '月'
      this.handleQuery()
    },
    handleQueryWeek() {
      this.query.maintainType = '周'
      this.handleQuery()
    },
    handleQueryApply() {
      this.query.maintainType = '报修'
      this.handleQuery()
    },
    // 根据登录用户角色获取角色类型列表
    getRoleTypes() {
