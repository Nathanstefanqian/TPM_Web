...
export default {
  ...
  methods: {
    ...
    async initUpdateAfter(row, data) {
      await this.getBuoys(data.companyId)
      data.taskBuoys.forEach((buoy, index, arr) => {
        const checkedBuoy = this.taskBuoys.buoys.find(f => f.key === buoy.imei)
        checkedBuoy.checked = true
        checkedBuoy.depth = buoy.depth
      })
    },
    ...
    getBuoys(companyId) {
      return api.buoy.getSelectlistByCompanyId(companyId).then(response => {
